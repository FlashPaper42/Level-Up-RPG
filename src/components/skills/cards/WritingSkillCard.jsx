...
/**
 * WritingSkillCard - Handles the Writing skill with letter input challenges and combat
 * Features: Attack/Defend/Special/Heal actions, type image name input, 3-card battle layout
 */
const WritingSkillCard = ({
    config,
    data,
    themeData,
    isCenter,
            ...
            <>
                {ReactDOM.createPortal(
                    ...
                    >
                        <div className="battle-layout relative flex items-center justify-center pointer-events-none">
                                                    ...
                                                    <>
                                                            ...
                                                            <div className="flex items-center justify-center gap-2 flex-1 w-full h-full p-2">
                                                                {challenge?.images?.map((img, idx) => (
                                                                    <React.Fragment key={idx}>
                                                                        ...
                                                                        <SafeImage src={img} className="max-w-full max-h-full min-w-[60px] min-h-[60px] object-contain animate-bob flex-1" style={{ maxHeight: '100px' }} />
                                                                    </React.Fragment>
                                                                ))}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="relative w-full flex justify-center">
                                                            <input
                                                                ...
                                                                maxLength={challenge?.answer.replace(/\s/g, '').length}
                                                                disabled={isWrong}
                                                            />
                                                            <div className={`flex gap-1 flex-wrap justify-center ${isWrong ? 'animate-shake' : ''}`}>
                                                                {(() => {
                                                                    const answerNoSpaces = challenge?.answer.replace(/\s/g, '');
                                                                    const answerLength = answerNoSpaces.length;
                                                                    return challenge?.answer.split('').map((char, i) => {
                                                                        if (char === ' ') return <div key={i} className={`${answerLength > 6 ? 'w-5' : 'w-7'}`}></div>;
                                                                        let inputIndex = 0;
                                                                        for (let j = 0; j < i; j++) {
                            ...
                            <div className="battle-center absolute left-1/2 pointer-events-auto" style={{ transform: 'translateX(-50%)' }} onClick={(e) => e.stopPropagation()}>
                                ...
                                >
                                    ...
                                    <div className="absolute bottom-0 left-0 w-10 h-10 border-b-4 border-l-4 border-green-600"></div>
                                    <div className="absolute bottom-0 right-0 w-10 h-10 border-b-4 border-r-4 border-green-600"></div>

                                    <div className="bg-gradient-to-b from-green-800 to-green-900 p-4 border-b-4 border-slate-700 relative flex-shrink-0">
                                        <div className="text-green-200 text-xl font-black uppercase tracking-wider text-center" style={{ textShadow: '2px 2px 0 #000' }}>
                        ...
                        <div className="battle-exit-hint absolute bottom-20 left-1/2 -translate-x-1/2 text-yellow-400 text-2xl font-bold pointer-events-none z-50" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.8)' }}>Click outside to flee the battle</div>
                    </div>,
                    document.body
                )}
            </>
...
export default WritingSkillCard;
