...
/**
 * WritingSkillCard - Handles the Writing skill with letter input challenges
 */
                                ...
                                <div className={`flex gap-1 flex-wrap justify-center ${isWrong ? 'animate-shake' : ''}`}>
                                    {(() => {
                                        ...
                                        return challenge?.answer.split('').map((char, i) => {
                                            if (char === ' ') return <div key={i} className={`${answerLength > 6 ? 'w-7' : 'w-10'}`}></div>;
                                            let inputIndex = 0;
                                            for (let j = 0; j < i; j++) {
                                                if (challenge?.answer[j] !== ' ') inputIndex++;
                                            }
                                            return (
                                                <div key={i} className={`${answerLength > 6 ? 'w-7 h-9 text-lg' : 'w-10 h-12 text-2xl'} border-b-4 flex items-center justify-center font-mono font-bold text-white bg-black/20 rounded-t ${isWrong ? 'border-red-500 bg-red-900/30' : (inputIndex < letterInput.length ? 'border-green-500' : 'border-gray-600')}`}>
